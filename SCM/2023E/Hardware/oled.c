...
//void OLED_ShowFloatNum(unsigned char x, unsigned char y,float tem, unsigned char TextSize)
//{
//	static unsigned char buffer[16];  // 创建缓冲区
//	int len;
//	// 将整数转换为字符串
//	len = sprintf((char*)buffer, "%f", tem); 
//	buffer[len] = '\0';
//	// 显示温度值（小字体）
//	OLED_ShowStr(x, y, buffer, TextSize);
//}

//void OLED_ShowIntNum(unsigned char x, unsigned char y,int tem, unsigned char TextSize)
//{
//	static unsigned char buffer[16];  // 创建缓冲区
//	int len;
//	// 将整数转换为字符串
//	len = sprintf((char*)buffer, "%d", tem); 
//	buffer[len] = '\0';
//	// 显示温度值（小字体）
//	OLED_ShowStr(x, y, buffer, TextSize);
//}
